...
//        }
 ...
 // Validate incoming request
    $validatedData = $request->validate([
        'rdv_id' => ['required', 'exists:appointments,id'], // Appointment ID must exist
        'rdv_status' => ['required', 'numeric'],           // Appointment status
    ]);

    // Check if status requires waiting list action
    if ($request->rdv_status == 4) {
        // Get the last `num` for this appointment
        $lastNum = WaitingList::where('appointment_id', $request->rdv_id)
            ->max('num');

        // Increment `num` for the new record
        $newNum = $lastNum ? $lastNum + 1 : 1;

        // Create a new waiting list entry
        WaitingList::create([
            'appointment_id' => $request->rdv_id,
            'user_id' => Appointment::find($request->rdv_id)->user_id,
            'num' => $newNum,
            'status' => 'waiting', // Initial status for waiting list
        ]);
    }
    ...
    $appointment->hours = $request->hours ?? $appointment->hours; // Use hours from request or retain existing
    $appointment->save();
