...
public function destroyWaitingList() {
    ...
    try {
        ...
        // Move data from `waiting_list` to `waiting_list_archive`
        DB::table('waiting_list_archive')->insert(
            DB::table('waiting_list')->get()->map(function ($record) {
                return (array) $record;
            })->toArray()
        );
        ...
        return redirect()->back()->with('success', 'Waiting list data archived and table truncated successfully!');
    } catch (\Exception $e) {
        // Rollback transaction in case of an error
        DB::rollBack();

        return redirect()->back()->with('error', 'An error occurred while archiving the waiting list: ' . $e->getMessage());
    }
}
