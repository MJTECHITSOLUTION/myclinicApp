    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#statusfilter').on('change', function () {
                ...
                // Loop through the rows and show/hide based on the filter
                $('#mytable tbody tr').each(function () {
                    const row = $(this);
                    const statusCell = row.find('td:nth-child(5) label');

                    if (statusCell.length) {
                        const statusValue = statusCell.text().trim();

                        // Show or hide the row based on the selected status
                        if (selectedStatus === "" || statusValue.includes(selectedStatus)) {
                            row.show();
                        } else {
                            row.hide();
                        }
        ...
        $('#statusfilter').on('change', function () {
            ...
            $('#mytable tbody tr').each(function () {
                ...
                if (statusCell.length) {
                    ...
                    // Show or hide the row based on the selected status
                    if (selectedStatus === "" || statusCell.parent().data('status') == selectedStatus) {
                        row.show();
                    } else {
                        row.hide();
                    }
