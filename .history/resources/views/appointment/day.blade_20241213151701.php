...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function filterTable() {
            ...
            rows.forEach(row => {
                let statusCell = row.querySelector('td:nth-child(5)'); // Get the status cell
                if (statusCell) {
                     // Find the label element inside the status cell
                    let statusLabel = statusCell.querySelector('label');
                    if (statusLabel) {
                        let rowStatus = -1; // Default value if no status is matched
                            if (statusLabel.textContent.includes('Not Yet Visited')) {
                                rowStatus = 0;
                            }
                            else if (statusLabel.textContent.includes('Terminé')) {
                                rowStatus = 1;
                            }
                            else if (statusLabel.textContent.includes('Salle d\'attente')) {
                                rowStatus = 3;
                            }
                             else if (statusLabel.textContent.includes('Salle soin')) {
                                rowStatus = 4;
                            }
                        ...
                        row.style.display = 'none';// if ther is no label hide the row
    
                    }
                } else {
                        row.style.display = 'none'; // hide the row if we have problem
                }
        ...
        filterTable(); // Initial filtering on page load
    });
