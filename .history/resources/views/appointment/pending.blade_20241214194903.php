    ...
    <script>
        function showOptionsPopup(appointmentId) {
            ...
            Swal.fire({
                ...
                preConfirm: (value) => {
                    if (value === true) {
                        return 2; // Value for Anuller
                    } else {
                        return 4; // Value for Salle de soin
                    }
                }
            }).then((result) => {
                if (result.isConfirmed) {
                    // Handle the action based on the button clicked
                    const actionValue = result.value;
                    if (actionValue === 2) {
                        // Logic for cancelling the appointment
                        document.getElementById('rdv_id2').value = appointmentId; // Set the appointment ID
                        document.getElementById('rdv-form-cancel').submit(); // Submit the cancel form
                    } else if (actionValue === 4) {
                        // Logic for Salle de soin
                        document.getElementById('rdv_id4').value = appointmentId; // Set the appointment ID
                        document.getElementById('rdv-form-soin').submit(); // Submit the soin form
                    }
                }
            });
        }
