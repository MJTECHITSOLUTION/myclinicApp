            ...
            <div class="row">
                <div class="col-4">
                    <h6 class="m-0 font-weight-bold text-primary w-75 p-2">Salle d'attente </h6>
                     <!-- Button to delete all Waiting_list -->
                     <form action="{{ route('appointment.destroyWaitingList') }}" method="POST" class="float-right ml-2 mb-1">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn rounded-0 btn-danger btn-sm" title="Delete All Waiting List">
                            <i class="fa fa-trash"></i> <span class="d-none d-lg-inline-block">Supprimer toute la liste d'attente</span>
                        </button>
                    </form>
                </div>
                <div class="col-8">
