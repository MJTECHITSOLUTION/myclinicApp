...
Route::post('/update-switch-state', 'HomeController@updateSwitchState');
Route::get('/get-switch-state', 'HomeController@getSwitchState');

Route::get('/send-notify', function (\Illuminate\Http\Request $request) {
    try {
        event(new NotifyMessage($request->all()));
        return back()->with('success', 'Notification envoyée avec succès');
    } catch (\Exception $e) {
        return back()->with('error', 'Erreur lors de l\'envoi de la notification');
    }
})->name('send.notify');
...
Route::match(['get', 'post'], '/appointment/dayfilter', action: 'AppointmentController@dayfilter')->name('dayfilter.search');
...
Route::post('/categories/add', 'DrugController@add_cat')->name('categories.add');
Route::post('/categories/edit', 'DrugController@edit_cat')->name('categories.edit');
Route::post('/categories/delete', 'DrugController@delete_cat')->name('categories.delete');
...
Route::get('/get-medicaments-by-type', 'PrescriptionController@getDrugsByType')->name('get.medicaments.by.type');
...
//Route::get('/get-drugs-by-type', 'PrescriptionController@getDrugsByType')->name('get.drugs.by.type');
...
Route::get('/appointment/delete/{id}', 'AppointmentController@destroyWaiting')->where('id', '[0-9]+')->name('appointment.destroy');
Route::delete('/appointment/destroyWaitingList', 'AppointmentController@destroyWaitingList')->name('appointment.destroyWaitingList');
// Route::post('/update-active-status/{id}', 'AppointmentController@updateActiveStatus')->where('id', '[0-9]+')->name('appointment.updateActiveStatus');

Route::post('/update-active-status/{id}', 'AppointmentController@updateActiveStatus')->name('update-active-status');
Route::post('/appointment/updateWaiting', 'AppointmentController@updateWaiting')->name('appointment.updateWaiting');
Route::get('/appointment/searchByDate', 'AppointmentController@searchByDate')->name('appointment.searchByDate');



Route::get('/notifies', 'NotifyController@index')->name('notifies.index');
Route::post('/notifies', 'NotifyController@storeOrUpdate')->name('notifies.storeOrUpdate');
Route::delete('/notifies/{id}', 'NotifyController@destroy')->name('notifies.destroy');
